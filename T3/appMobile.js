...
function updatePlayer(){
    ...
    if (fwdValue > 0) {
        if ((airplane.getGeometry().position.z - game.cameraHolder.position.z) > -200 ) {
            ...
            airplane.getGeometry().position.addScaledVector(
                tempVector,
                1.5
            )
        }
    ...
    if (lftValue > 0) {
        if ((airplane.getGeometry().position.x - game.cameraHolder.position.x) > -70 ) {
            ...
            gsap.to(airplane.model.rotation, { z: degreesToRadians(135), duration: 0.5 });
            airplane.isRotated = true;
        }
    }
    ...
    if (rgtValue > 0) {
        if ((airplane.getGeometry().position.x - game.cameraHolder.position.x) < 70 ) {
            ...
            gsap.to(airplane.model.rotation, { z: degreesToRadians(225), duration: 0.5 });
            airplane.isRotated = true;
        }
    }
...
async function addJoysticks(){
    ...
    joystickL.on('move', async function (evt, data) {
        ...
        if (turn == 0) {
            gsap.to(airplane.model.rotation, { z: degreesToRadians(180), duration: 0.5 });
            await sleep(250);
            airplane.isRotated = false;
        }
